...
func TestParseTestSources(t *testing.T) {
	descr, err := parseTestSources([]string{"tools/please_go_tool/testmain/test_data/example_test.go"})
...
func TestParseTestSourcesWithMain(t *testing.T) {
	descr, err := parseTestSources([]string{"tools/please_go_tool/testmain/test_data/example_test_main.go"})
	assert.NoError(t, err)
...
func TestWriteTestMain(t *testing.T) {
	err := WriteTestMain(
		"tools/please_go_tool/testmain/test_data",
		false, // not version 1.8
		[]string{"tools/please_go_tool/testmain/test_data/example_test.go"},
		"test.go",
		[]CoverVar{},
	)
	assert.NoError(t, err)
